...
const hrNav = [
  { href: "/hr_portal/dashboard",  label: "Dashboard",  icon: LayoutDashboard },
  { href: "/hr_portal/candidates", label: "Candidates", icon: Users },
  { href: "/hr_portal/jobs",       label: "Jobs",        icon: Briefcase },
...
export default function Sidebar() {
  ...
  const isManager = path.startsWith("/Manager_Portal");
  const nav = isManager ? managerNav : hrNav;
  const roleLabel = isManager ? "Hiring Manager" : "AI-Powered Recruiting";
  ...
  const logoBlock = (
    <div style={{ padding: "18px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div>
        ...
        <div style={{ fontSize: 11, color: C.textSub }}>{roleLabel}</div>
      </div>
    </div>
  );
